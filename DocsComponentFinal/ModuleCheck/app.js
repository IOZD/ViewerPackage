...
let tlogDocsService = new TlogDocsService({
    isUS: _isUS,
    local: 'he-IL'
});

var OrderDocumentsDict = {};

// get all docs.
let docs = tlogDocsService.getDocs(tlogObject, printData);

// create doc request from the server.(for the different asynchronous printData(bill/check/etc))
var docsArray = createDocRequests(docs);



//**************************************************//
//           createDocRequests Example              //
//**************************************************//
...
function createDocRequests(docs) {
    ...
    docs.forEach(doc => {
        ...
        //**************************************************//
        //           when you'll have async Requests        //
        //**************************************************//
        // if (doc.type !== "check" && doc.type !== "tlog") {
        //     if (!_isUS) {
        //         getPrintData(doc)
        //             .then(printData => {
        //                 $ctrl.OrderDocumentsDict[doc.id].inProgress = false;
        //                 $ctrl.OrderDocumentsDict[doc.id].data = printData[0];
        //             }).catch(err => console.log(err));
        //     }
        //     else {
        //         OrderDocumentsDict[doc.id].inProgress = false;
        //         OrderDocumentsDict[doc.id].data = printData;
        //     }
        // }
        // else if (doc.type === "check") {
        //     if (!isLoadChecks) {

        //         isLoadChecks = true;
        //         getChecksData(doc)
        //             .then(checkPrintData => {
        //                 if (checkPrintData && checkPrintData.length > 1) {
        //                     checkPrintData.forEach(printData => {
        //                         let checkNumber = printData.printData.variables.CHECK_NO;
        //                         let checkDoc = docs.find(c => c.md && c.md.checkNumber === checkNumber && c.type === "check");

        //                         if (checkDoc) {
        //                             $ctrl.OrderDocumentsDict[checkDoc.id].inProgress = false;
        //                             $ctrl.OrderDocumentsDict[checkDoc.id].data = printData;
        //                         }
        //                     });
        //                 }
        //             });
...
//*****************************************************************//
// asynchronous requests Example  - our local implementation       //
//*****************************************************************//
// function getPrintData(doc) {
//     return OrdersDAC.getPrintData(doc.id)
//         .then(docData => {
//             return docData;
//         });
// }

// function getChecksData(doc) {
//     return CheckDAC.get(doc.tlogId)
//         .then(docData => {
//             console.log('docData')
//             console.log(docData)
//             return docData;
...
tpl = tlogDocsService.getTemplate(firstDoc, printData, printData);




var divForInsertedTemplate = document.getElementById('checkTemplateDiv');
divForInsertedTemplate.insertAdjacentHTML('beforeend', tpl);



//**************************************************************************//
// when you'll have buttons , it would be the suggested implementation      //
//**************************************************************************//


buttonClicked = function (button) {

    if (OrderDocumentsDict) {

        // is Order (Main Bill)
        if (button.isFullOrder) {

            tpl = tlogDocsService.getTemplate(button, serverPrintData, serverPrintData);
        }
        else {

            let selectedDoc = OrderDocumentsDict[button.id];
            ...
            if (button.tlogId) {
                ...
                tpl = tlogDocsService.getTemplate(button, {
                    documentType: docData.documentType,
                    organizationId: docData.organizationId,
                    printData: {
                        collections: docData.printData.collections,
                        variables: docData.printData.variables
                    },
                }, serverPrintData);
            }
        }
